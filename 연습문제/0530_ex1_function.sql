/*2023-05-30 함수 연습*/
/*고객들 평균 주문 금액을 백원단위로 반올림하기*/
select custid "고객번호", round(sum(saleprice)/count(*),-2) "평균금액" from orders group by custid;

/*날짜의 이전 이후 계산*/
select TO_DATE('2020-07-01','yyyy-mm-dd')+5 before, to_date('2020-07-01','yyyy-mm-dd')-5 after from dual;

/*주문일로부터 10일 후 매출 확정*/
select orderid " 주문번호", orderdate "주문일", ordersate+10 "확정" from Orders;

/*dbms서버에 설정된 현재 날짜와 시간, 요일 확인*/
select sysdate, To_char(sysadte,'yyyy/mm/dd hh24:mi:ss')"sysdate_1" from dual;
